...
angular.module('angularCharts').directive('acChart', [
   ...
   */
     ...
     */
      function pieChart() {
        var radius = Math.min(height,width)/2.25;
        var svg = d3.select(chartContainer[0]).append('svg').attr('width', width).attr('height', height).append('g').attr('transform', 'translate(' + width / 2.5 + ',' + height / 2 + ')');
        var innerRadius = 0;
        ...
        path.append('path').style('fill', function (d, i) {
          ...
          if (!complete) {
            ...
            path.on('mouseenter', function (d) {
              makeToolTip({ value: d.tooltip ? d.tooltip : d.data.tooltip }, d3.event);
              d3.select(this).select('path').transition().duration(200).attr("d",arcOver);
              config.mouseover(d, d3.event);
              scope.$apply();
            }).on('mouseleave', function (d) {
              d3.select(this).select('path').transition().duration(200).attr("d",arc).style('stroke', '').style('stroke-width', '');
              removeToolTip();
              config.mouseout(d, d3.event);
              scope.$apply();
            }).on('mousemove', function (d) {
